...
function setupLayerDialogEvents() {
    ...
    if (renameApplyBtn) renameApplyBtn.addEventListener('click', applyLayerRename);
    if (renameCancelBtn) renameCancelBtn.addEventListener('click', closeLayerRenameDialog);
    if (renameNewLayer) {
...
export function openLayerDialog() {
    ...
    const modeRadio = document.querySelector(`input[name="dialog-layer-filter-mode"][value="${layerDialogState.filterMode}"]`);
...
export function loadLayerView(viewId) {
    ...
    // Update UI elements
    if (typeof window.updateLayerSummary === 'function') window.updateLayerSummary();
