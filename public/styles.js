...
/**
 * Calculate edge color based on weight (lighter for higher weights)
 * @param {number} weight - Edge weight (0.1-30)
 * @param {boolean} isSelected - Whether edge is selected
 * @returns {string} Edge color
 */
export function getEdgeColor(weight, isSelected) {
  if (isSelected) {
    return '#F4A460'; // Sandy brown for selected edges
  }
  
  // Normalize weight to 0-1 range (inverted: lower weight = darker)
  const clampedWeight = Math.max(0.1, Math.min(30, weight));
  const normalized = 1 - Math.max(0, Math.min(1, (clampedWeight - 0.1) / 29.9));
  
  // Map to color range: darker (lower weight) to lighter (higher weight)
  // Base color: #EFF0E9 (light gray), darker: #B0B0B0 (medium gray)
  const lightness = 0.7 + normalized * 0.2; // 0.7 to 0.9
  const grayValue = Math.floor(240 * lightness);
  
  return `rgb(${grayValue}, ${grayValue}, ${grayValue})`;
}

/**
 * Calculate edge label background color based on weight
 * @param {number} weight - Edge weight
 * @returns {string} Background color with alpha
 */
export function getEdgeLabelBackgroundColor(weight) {
  const clampedWeight = Math.max(0.1, Math.min(30, weight));
  const normalized = Math.max(0, Math.min(1, (clampedWeight - 0.1) / 29.9));
  
  // Lighter background for higher weights (more transparent)
  const alpha = 0.85 - normalized * 0.15; // 0.85 to 0.7
